...
// Add services
Shopwell.Service().register('acl', () => aclService);
Shopwell.Service().register('feature', () => feature);
Shopwell.Feature = Shopwell.Service('feature');
Shopwell.Service().register('repositoryFactory', () => repositoryFactory);

// Provide all services
Shopwell.Service().list().forEach(serviceKey => {
    config.global.provide[serviceKey] = Shopwell.Service(serviceKey);
});

// Set important functions for Shopwell Core
...
// Prepare Context
Shopwell.Store.get('context').api.installationPath = 'installationPath';
Shopwell.Store.get('context').api.apiPath = '/api';
Shopwell.Store.get('context').api.apiResourcePath = '/api/v3';
Shopwell.Store.get('context').api.assetsPath = '';
Shopwell.Store.get('context').api.languageId = '2fbb5fe2e29a4d70aa5854ce7ce3e20b';
Shopwell.Store.get('context').api.inheritance = false;
Shopwell.Store.get('context').api.systemLanguageId = '2fbb5fe2e29a4d70aa5854ce7ce3e20b';
Shopwell.Store.get('context').api.liveVersionId = '0fa91ce3e96a4bc2be4bd9ce752c3425';
...
// disable telemetry
Shopwell.Telemetry.initialize = () => Promise.resolve();
...
const directiveRegistry = Shopwell.Directive.getDirectiveRegistry();
