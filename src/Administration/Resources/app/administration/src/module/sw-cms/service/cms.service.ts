...
class CmsService {
    ...
    public registerCmsElement(config: CmsElementConfig): boolean {
        ...
        Shopwell.Application.view?.setReactive(this.elementRegistry, config.name, config);
    ...
    public getEntityMappingTypes(entityName: string) {
        const schema = Shopwell.EntityDefinition.has(entityName) ? Shopwell.EntityDefinition.get(entityName) : undefined;
    ...
    private async addCustomFieldsToMappingTypes(entityName: string, mappings: EntityMappings): Promise<void> {
        const customFieldRepository = Shopwell.Service('repositoryFactory').create('custom_field');
        const criteria = new Shopwell.Data.Criteria(1, 50);
        criteria.addFilter(Shopwell.Data.Criteria.equals('customFieldSet.relations.entityName', entityName));
        criteria.addFilter(Shopwell.Data.Criteria.equals('active', 1));
        criteria.addFilter(
            Shopwell.Data.Criteria.multi('OR', [
                Shopwell.Data.Criteria.equals('type', 'text'),
                Shopwell.Data.Criteria.equals('type', 'datetime'),
                Shopwell.Data.Criteria.equals('type', 'html'),
            ]),
        );

        const customFields = await customFieldRepository.search(criteria, Shopwell.Context.api);
    ...
    private handleAssociationMapping(
        ...
        if (toOneAssociation && propSchema.entity) {
            ...
            if (deep) {
                const schema = Shopwell.EntityDefinition.get(propSchema.entity);
    ...
    static getEntityData(slot: RuntimeSlot, configKey: string): CmsSlotData {
        ...
        const entityData: CmsSlotData = {
            value: [],
            key: configKey,
            searchCriteria: new Shopwell.Data.Criteria(1, 25),
            ...entity,
        };
        ...
        entityData.searchCriteria = entity?.criteria ? entity.criteria : new Shopwell.Data.Criteria(1, 25);
    ...
    Object.keys(slot.config).forEach((configKey) => {
        ...
        if (entity && value) {
            ...
            const entityData: CmsSlotData = {
                value: [value].flat(),
                key: configKey,
                searchCriteria: entity.criteria ? entity.criteria : new Shopwell.Data.Criteria(1, 25),
                ...entity,
            };
