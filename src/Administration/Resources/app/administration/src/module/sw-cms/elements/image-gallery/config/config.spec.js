...
describe('src/module/sw-cms/elements/image-gallery/config', () => {
    ...
    it('should resolve media upload payloads via repository', async () => {
        ...
        expect(wrapper.vm.element.config.sliderItems.value).toEqual([
            {
                mediaId: 'uploaded-id',
                mediaUrl: 'http://shopwell.com/uploaded-id.jpg',
                newTab: false,
                url: null,
            },
        ]);
    ...
    it('should remove previous mediaItem if it already exists after upload', async () => {
        ...
        expect(wrapper.vm.element.config.sliderItems.value[0].mediaUrl).toBe('http://shopwell.com/image1.jpg');
        ...
        await wrapper.vm.onImageUpload({
            ...
            url: 'http://shopwell.com/image1-updated.jpg',
        });

        // Should still only have one item and the URL should be updated
        expect(wrapper.vm.element.config.sliderItems.value).toHaveLength(1);
        expect(wrapper.vm.element.config.sliderItems.value[0].mediaUrl).toBe('http://shopwell.com/image1-updated.jpg');
    });
});
