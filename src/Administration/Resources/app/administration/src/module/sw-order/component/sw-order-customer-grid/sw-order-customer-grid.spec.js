...
describe('src/module/sw-order/view/sw-order-customer-grid', () => {
    beforeAll(() => {
        Shopwell.Service().register('contextStoreService', () => {
            return {
        ...
        Shopwell.Service().register('cartStoreService', () => {
            return {
                getCart: () =>
                    Promise.resolve({
                        data: {
        ...
        Shopwell.Store.get('swOrder').setCart({ token: '', lineItems: [] });
        Shopwell.Store.get('swOrder').setContext({ customer: {} });
    ...
    it('should update customer context and cart after selecting a customer', async () => {
        setCustomerData(customers);
        Shopwell.Store.get('swOrder').setCartToken('1d8af3ddddbd378ba0065debd5e4e4b1');
    ...
    it('should check customer initially if customer exists', async () => {
        ...
        Shopwell.Store.get('swOrder').setCustomer({
            ...customers[0],
        });
    ...
    it('should reset language to default if system language exists in customer sales channel languages', async () => {
        ...
        Shopwell.Store.get('swOrder').setCartToken('HE6KD7HOCC3TCS0AX903KCA6JHXCTXU2');

        const resetLanguageToDefaultSpy = jest.spyOn(Shopwell.Store.get('context'), 'resetLanguageToDefault');
