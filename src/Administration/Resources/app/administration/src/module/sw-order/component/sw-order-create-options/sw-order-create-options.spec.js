...
describe('src/module/sw-order/view/sw-order-create-options', () => {
    beforeAll(() => {
        ...
        Shopwell.Store.get('swOrder').setCart(cart);
        Shopwell.Store.get('swOrder').setContext(context);
        Shopwell.Store.get('swOrder').setCustomer(customerData);

        if (Shopwell.Store.get('context')) {
            Shopwell.Store.unregister('context');
        }

        Shopwell.Store.register(contextStore);
    });
    ...
    it('should update context when selecting shipping method', async () => {
        ...
        Shopwell.Service('cartStoreService').getCart = jest.fn(() =>
            Promise.resolve({
                data: {
                    lineItems: [],
                    deliveries: [
    ...
    it('should be set context language when language selected', async () => {
        ...
        expect(Shopwell.Store.get('context').api.languageId).toBe(contextStore.state().api.languageId);
