...
const mockRepositoryFactory = (entity, mailTemplates) => {
    if (entity === 'mail_template') {
        return {
            search: jest.fn(() =>
                Promise.resolve(new EntityCollection('', '', Shopwell.Context.api, null, mailTemplates, 2)),
            ),
    ...
    if (entity === 'mail_header_footer') {
        return {
            search: (criteria) => {
                ...
                return Promise.resolve(new EntityCollection('', '', Shopwell.Context.api, null, [mockMailHeaderFooter], 1));
            },
        };
    }
    return {};
