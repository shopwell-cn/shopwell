...
const { intersectionBy, chunk, uniqBy } = Shopwell.Utils.array;
...
export default {
    ...
    methods: {
        async createdComponent() {
            ...
            Shopwell.Store.get('swBulkEdit').resetOrderDocumentsIsChanged();

            this.isLoading = true;

            this.order = this.orderRepository.create(Shopwell.Context.api);
            ...
            const requests = payloadChunks.map((ids) => {
                ...
                criteria.addFilter(
                    Criteria.multi('AND', [
                        ...
                        Criteria.equals(versionField, Shopwell.Context.api.liveVersionId),
                    ]),
                );
        ...
        fetchToStateMachineTransitions(states) {
            ...
            return this.stateMachineStateRepository
                .search(this.toStateMachineStatesCriteria(states), Shopwell.Context.api)
                .then((response) => {
                    if (!response.length) {
                        return [];
                    }
