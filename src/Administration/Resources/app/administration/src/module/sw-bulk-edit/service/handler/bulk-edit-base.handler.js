...
const { getObjectDiff } = Shopwell.Utils.object;
...
class BulkEditBaseHandler {
    constructor() {
        this.syncService = Shopwell.Service('syncService');
        this.repositoryFactory = Shopwell.Service('repositoryFactory');
    ...
    async buildBulkSyncPayload(changes) {
        const definition = Shopwell.EntityDefinition.get(this.entityName);
        ...
        await Promise.all(
            changes.map(async (change) => {
                ...
                if (!field) {
                    Shopwell.Utils.debug.warn('Entity factory', `Property ${this.entityName}.${change.field} not found`);
