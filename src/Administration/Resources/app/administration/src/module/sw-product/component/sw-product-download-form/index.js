...
export default {
    ...
    computed: {
        product() {
            const state = Shopwell.Store.get('swProductDetail');
        ...
        error() {
            return Shopwell.Store.get('error').getApiError(this.product, 'downloads');
        },
    ...
    methods: {
        ...
        async successfulUpload({ targetId }) {
            ...
            if (this.error) {
                Shopwell.Store.get('error').removeApiError(this.error.selfLink);
            }
        },
