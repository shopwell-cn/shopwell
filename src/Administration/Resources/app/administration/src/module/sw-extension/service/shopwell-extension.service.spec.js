 ...
 * @sw-package checkout
 */
describe('src/module/sw-extension/service/shopwell-extension.service', () => {
    let shopwellExtensionService;
    ...
    describe('it delegates lifecycle methods', () => {
        ...
        function expectUpdateExtensionDataCalled() {
            ...
            expect(Shopwell.Store.get('shopwellExtensions').myExtensions.data).toEqual(['new extensions']);
            expect(Shopwell.Store.get('shopwellExtensions').myExtensions.loading).toBe(false);
        ...
        function expectUpdateModulesCalled() {
            ...
            expect(Shopwell.Store.get('shopwellApps').apps).toEqual([
                'new app modules',
            ]);
        }
    ...
    describe('checkLogin', () => {
        const checkLoginSpy = jest.spyOn(Shopwell.Service('storeService'), 'checkLogin');

        beforeEach(() => {
            Shopwell.Store.get('shopwellExtensions').userInfo = true;
        });

        it.each([
            [{ userInfo: { email: 'user@shopwell.com' } }],
            [{ userInfo: null }],
        ])('sets login status depending on checkLogin response', async (loginResponse) => {
            checkLoginSpy.mockImplementationOnce(() => loginResponse);

            await shopwellExtensionService.checkLogin();

            expect(Shopwell.Store.get('shopwellExtensions').userInfo).toStrictEqual(loginResponse.userInfo);
        });
        ...
        it('sets login status to false if checkLogin request fails', async () => {
            ...
            expect(Shopwell.Store.get('shopwellExtensions').loginStatus).toBe(false);
            expect(Shopwell.Store.get('shopwellExtensions').userInfo).toBeNull();
        });
    });
    ...
    describe('isVariantDiscounted', () => {
        it('returns true if price is discounted and campaign is active', async () => {
            ...
            expect(shopwellExtensionService.isVariantDiscounted(variant)).toBe(true);
        });
        ...
        it('returns false if price is discounted but campaign is not active', async () => {
            ...
            Shopwell.Service('shopwellDiscountCampaignService').isDiscountCampaignActive.mockImplementationOnce(() => false);

            expect(shopwellExtensionService.isVariantDiscounted(variant)).toBe(false);
        });

        it('returns false if variant is falsy', async () => {
            expect(shopwellExtensionService.isVariantDiscounted(null)).toBe(false);
        });

        it('returns false if variant has no discountCampaign', async () => {
            expect(shopwellExtensionService.isVariantDiscounted({})).toBe(false);
        });

        it('returns false if discounted price is net price', async () => {
            expect(
                shopwellExtensionService.isVariantDiscounted({
                    netPrice: 100,
                    discountCampaign: {
                        discountedPrice: 100,
                    },
        ...
        it('orders variants by recommendation and discounting', async () => {
            ...
            shopwellExtensionService
                ...
                .forEach((current, currentIndex, orderedVariants) => {
                    const isCurrentDiscounted = shopwellExtensionService.isVariantDiscounted(current);
                    const currentRecommendation = shopwellExtensionService.mapVariantToRecommendation(current);

                    orderedVariants.forEach((comparator, comparatorIndex) => {
                        const isComparatorDiscounted = shopwellExtensionService.isVariantDiscounted(comparator);
                        const comparatorRecommendation = shopwellExtensionService.mapVariantToRecommendation(comparator);
    ...
    describe('getPriceFromVariant', () => {
        it('returns discounted price if variant is discounted', async () => {
            expect(
                shopwellExtensionService.getPriceFromVariant({
                    netPrice: 100,
                    discountCampaign: {
                        discountedPrice: 80,
                    },
    ...
    describe('mapVariantToRecommendation', () => {
        it.each([
            ...
            expect(shopwellExtensionService.mapVariantToRecommendation({ type })).toBe(expectedRecommendation);
        });
    });
    ...
    describe('getOpenLink', () => {
        it('returns always a open link for theme', async () => {
            ...
            const openLink = await shopwellExtensionService.getOpenLink({
                isTheme: true,
                type: shopwellExtensionService.EXTENSION_TYPES.APP,
                name: 'SwagExampleApp',
            });
        ...
        it('returns valid open link for app with main module', async () => {
            Shopwell.Store.get('shopwellApps').apps = appModulesFixtures;

            expect(
                await shopwellExtensionService.getOpenLink({
                    isTheme: false,
                    type: shopwellExtensionService.EXTENSION_TYPES.APP,
                    name: 'testAppA',
                }),
        ...
        it('returns no open link for plugins not registered', async () => {
            expect(
                await shopwellExtensionService.getOpenLink({
                    isTheme: false,
                    type: shopwellExtensionService.EXTENSION_TYPES.PLUGIN,
                    name: 'SwagNoModule',
                }),
            ).toBeNull();
        });
