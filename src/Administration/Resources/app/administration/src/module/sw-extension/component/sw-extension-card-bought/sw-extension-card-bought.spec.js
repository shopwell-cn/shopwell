...
Shopwell.Application.addServiceProvider('loginService', () => {
    return {
...
Shopwell.Application.addServiceProvider('extensionStoreActionService', () => {
...
Shopwell.Application.addServiceProvider('extensionErrorService', () => {
...
async function createWrapper(extension) {
    return mount(await wrapTestComponent('sw-extension-card-bought', { sync: true }), {
        global: {
            ...
            mixins: [
                Shopwell.Mixin.getByName('sw-extension-error'),
            ],
            stubs: {
...
describe('src/module/sw-extension/component/sw-extension-card-bought', () => {
    ...
    it('should link to the detail page', async () => {
        ...
        expect(detailLink.props('routerLink')).toStrictEqual({
    ...
    it('should display error on install and download attempt when app subscription is expired', async () => {
                        ...
                        errors: [
                            {
                                ...
                                meta: {
                                    documentationLink: 'https://docs.shopwell.com/en/shopwell-6-en',
                                },
                                status: '500',
        ...
        expect(wrapper.find('.sw-extension-card-bought__installation-failed-modal h3 + p').text()).toBe(
            'The download of the extension is not allowed, please purchase a corresponding license or contact the customer service',
        );
        expect(wrapper.find('.sw-extension-card-bought__installation-failed-modal p > a').text()).toBe(
            'https://docs.shopwell.com/en/shopwell-6-en',
        );
    });
