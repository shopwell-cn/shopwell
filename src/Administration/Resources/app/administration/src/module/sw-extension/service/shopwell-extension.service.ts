...
import type { ShopwellDiscountCampaignService } from 'src/app/service/discount-campaign.service';
import type {
 ...
 * @private
 */
export default class ShopwellExtensionService {
    ...
    constructor(
        ...
        private readonly discountCampaignService: ShopwellDiscountCampaignService,
    ...
    public async updateExtensionData(refreshExtensions: boolean = true): Promise<void> {
        Shopwell.Store.get('shopwellExtensions').loadMyExtensions();

        try {
            if (!Shopwell.Store.get('context').app.config?.settings?.disableExtensionManagement && refreshExtensions) {
            ...
            Shopwell.Store.get('shopwellExtensions').setMyExtensions(myExtensions);

            await this.updateModules();
        } finally {
            Shopwell.Store.get('shopwellExtensions').setLoading(false);
        }
    }
    ...
    public async getOpenLink(extension: Extension): Promise<null | LabeledLocation | RouteLocationNamedRaw> {
        ...
        const entryRoutes = Shopwell.Store.get('extensionEntryRoutes').routes;
    ...
    private async getLinkToTheme(extension: Extension) {
        ...
        const themeRepository = Shopwell.Service('repositoryFactory').create('theme');
    ...
    private getAppFromStore(extensionName: string) {
        return Shopwell.Store.get('shopwellApps').apps.find((innerApp) => {
