...
export default {
    ...
    methods: {
        ...
        getUserConfig() {
            return this.userConfigRepository.search(this.userConfigCriteria, Shopwell.Context.api).then((response) => {
                if (response.length) {
                    this.pluginUploadUserConfig = response.first();
                } else {
                    this.pluginUploadUserConfig = this.userConfigRepository.create(Shopwell.Context.api);
        ...
        saveConfig(value) {
            ...
            this.userConfigRepository.save(this.pluginUploadUserConfig, Shopwell.Context.api).then(() => {
                this.getUserConfig();
            });
        },
    },
