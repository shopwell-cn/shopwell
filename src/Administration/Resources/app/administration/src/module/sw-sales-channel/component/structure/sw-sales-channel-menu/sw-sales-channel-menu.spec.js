...
const headlessSalesChannel = {
    ...
    domains: [],
    type: {
        id: Shopwell.Defaults.apiSalesChannelTypeId,
...
const storeFrontWithStandardDomain = {
    ...
    domains: [
        ...
        {
            languageId: Shopwell.Defaults.systemLanguageId,
            url: 'http://shop/default-language',
        },
    ],
    type: {
        id: Shopwell.Defaults.storefrontSalesChannelTypeId,
...
Shopwell.Application.addServiceProvider('salesChannelFavorites', () => {
    const favorites = [];
...
describe('src/module/sw-sales-channel/component/structure/sw-sales-channel-menu', () => {
    beforeEach(async () => {
        Shopwell.Service('salesChannelFavorites').state.favorites = [];
        Shopwell.Store.get('session').languageId = defaultAdminLanguageId;
    ...
    it('takes first domain link if neither default language nor admin language exists', async () => {
        window.open = jest.fn();
        Shopwell.Store.get('session').languageId = Shopwell.Utils.createId();
    ...
    it('shows "more" when more than 50 sales channels are available and marked as favourites', async () => {
        ...
        Shopwell.Service('salesChannelFavorites').state.favorites = salesChannels.map((el) => el.id);
    ...
    it.each([
        ...
        Shopwell.Utils.EventBus.emit(eventName);
        await flushPromises();
