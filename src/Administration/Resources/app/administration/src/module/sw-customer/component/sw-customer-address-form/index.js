...
const { Defaults, EntityDefinition } = Shopwell;
...
export default {
    ...
    watch: {
        ...
        'country.forceStateInRegistration'(newVal) {
            if (!newVal) {
                Shopwell.Store.get('error').removeApiError(
        ...
        'country.postalCodeRequired'(newVal) {
            if (!newVal) {
                Shopwell.Store.get('error').removeApiError(`${this.address.getEntityName()}.${this.address.id}.zipcode`);
            }
