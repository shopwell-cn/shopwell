...
export default {
    ...
    computed: {
        currentUserId() {
            return Shopwell.Store.get('session').currentUser.id;
        },
    ...
    methods: {
        ...
        loadUserConfig() {
            return this.userConfigRepository.search(this.userConfigCriteria, Shopwell.Context.api).then((userConfigs) => {
                if (userConfigs.length === 0) {
                    this.userConfig = this.userConfigRepository.create(Shopwell.Context.api);
        ...
        onSave() {
            ...
            return this.countryRepository
                .save(this.country, Shopwell.Context.api)
                .then(() => {
                    if (userConfigValue && Object.keys(userConfigValue).length > 0) {
                        this.userConfigRepository.save(this.userConfig, Shopwell.Context.api).then(() => {
                            this.loadUserConfig();
                        });
                    }
                    this.loadEntityData();
        ...
        onUpdateCountry(path, value) {
            Shopwell.Utils.object.set(this.country, path, value);
        },
    },
};
