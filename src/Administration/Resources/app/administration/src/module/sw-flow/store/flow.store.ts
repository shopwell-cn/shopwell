...
type EntityActionName = (typeof Shopwell.Constants.FLOW.ACTION)[EntityActions];

const swFlowStore = Shopwell.Store.register('swFlow', {
    state: () => ({
        flow: {
            eventName: '',
    ...
    actions: {
        ...
        setOriginFlow(flow: Flow) {
            const clonedFlow = Shopwell.Utils.object.cloneDeep(flow);
            ...
            flow.sequences.forEach((item) => {
                sequences.add(Shopwell.Utils.object.cloneDeep(item) as Sequence);
            });
        ...
        updateSequence(params: Partial<Sequence> & { id: string }) {
            ...
            this.flow.sequences = new EntityCollection(sequences.source, sequences.entity, Shopwell.Context.api, null, [
