...
export default {
    ...
    computed: {
        shippingMethod() {
            return Shopwell.Store.get('swShippingDetail').shippingMethod;
        },

        currencies() {
            return Shopwell.Store.get('swShippingDetail').currencies;
        },

        restrictedRuleIds() {
            return Shopwell.Store.get('swShippingDetail').restrictedRuleIds;
        },
    ...
    methods: {
        ...
        loadCurrencies() {
            ...
            this.currencyRepository.search(criteria, Context.api).then((currencyResponse) => {
                Shopwell.Store.get('swShippingDetail').currencies = this.sortCurrencies(currencyResponse);
        ...
        loadEntityData() {
            ...
            this.shippingMethodRepository
                .get(this.shippingMethodId, Shopwell.Context.api, this.shippingMethodCriteria)
                .then((res) => {
                    Shopwell.Store.get('swShippingDetail').shippingMethod = res;

                    this.ruleConditionDataProviderService.getRestrictedRules('shippingMethodPrices').then((result) => {
                        Shopwell.Store.get('swShippingDetail').restrictedRuleIds = this.restrictedRuleIds.concat(result);
                    });
