...
export default {
    ...
    data() {
        return {
            ...
            availableVariables: {},
            entitySchema: Object.fromEntries(Shopwell.EntityDefinition.getDefinitionRegistry()),
    ...
    methods: {
        ...
        loadEntityData() {
            ...
            this.isLoading = true;
            this.mailTemplateRepository.get(this.mailTemplateId, Shopwell.Context.api, criteria).then((item) => {
        ...
        createMediaCollection() {
            return new EntityCollection('/media', 'media', Shopwell.Context.api);
        },
        ...
        getMailTemplateMedia() {
            ...
            this.mailTemplate.media.forEach((mediaAssoc) => {
                if (mediaAssoc.languageId === Shopwell.Context.api.languageId) {
        ...
        onClickTestMailTemplate() {
            ...
            this.salesChannelRepository.get(this.testMailSalesChannelId, Context.api, criteria).then((salesChannel) => {
                if (!salesChannel.languages.has(Shopwell.Context.api.languageId)) {
        ...
        createMailTemplateMediaAssoc(mediaItem) {
            ...
            mailTemplateMedia.languageId = Shopwell.Context.api.languageId;
        ...
        _checkIfMediaIsAlreadyUsed(mediaId) {
            return this.mailTemplate.media.some((mailTemplateMedia) => {
                return (
                    mailTemplateMedia.mediaId === mediaId && mailTemplateMedia.languageId === Shopwell.Context.api.languageId
                );
            });
        },
        ...
        loadAvailableVariables(variable, variableEntitySchema) {
            ...
            const foundVariables = Object.keys(Shopwell.Utils.get(this.mailTemplateType.templateData, variable));

            const keys = foundVariables.map((val) => {
                const availableVariable = Shopwell.Utils.get(this.mailTemplateType.templateData, variablePath.concat(val));
        ...
        isToManyAssociationVariable(variable) {
            ...
            const field = Shopwell.Utils.get(this.entitySchema, `${variables.join('.')}`);
        ...
        loadInitialAvailableVariables() {
            ...
            Object.keys(this.mailTemplateType.templateData).forEach((variable) => {
                const availableVariable = Shopwell.Utils.get(this.mailTemplateType.templateData, variable);
                let length = 0;
