...
const { EntityCollection, Criteria } = Shopwell.Data;
...
function getCollection(repository, entities = [], aggregations = defaultAggregations, total = 0) {
    return new EntityCollection(
        ...
        repository,
        Context.api,
        { isShopwellContext: true, page: 1, limit: 25 },
        entities,
        total,
        aggregations,
    );
...
describe('src/module/sw-settings-rule/page/sw-settings-rule-detail', () => {
    ...
    it.each([
        ...
        const apiLanguageId = Shopwell.Store.get('context').api.languageId;
        expect(Shopwell.Store.get('context').api.languageId).not.toBe('uuid1');
        ...
        expect(Shopwell.Store.get('context').api.languageId).toBe(abort ? apiLanguageId : 'uuid1');

        // cleanup
        Shopwell.Store.get('context').api.languageId = apiLanguageId;
    });
