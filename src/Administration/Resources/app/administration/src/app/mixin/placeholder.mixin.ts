    ...
    defineComponent({
        methods: {
            placeholder<EntityName extends keyof EntitySchema.Entities>(
                ...
                if (Shopwell.Utils.types.isString(entity[field]) && entity[field].length > 0) {
                    return entity[field];
                }

                // Return the field from parent translation if set
                const parentLanguageId = Shopwell.Context.api.language ? Shopwell.Context.api.language.parentId : null;
