...
export default class RuleConditionService {
    ...
    private getTransformedBooleanFieldConfig(transformedConfig: CustomFieldConditionConfig) {
        const locale = Shopwell.Store.get('session')?.currentLocale || 'en-GB';
        const app = Shopwell.Application.getApplicationRoot();
    ...
    getRestrictedRules(entityName: string) {
        ...
        const ruleRepository = Shopwell.Service('repositoryFactory').create('rule');
