...
export default class FilterService {
    ...
    getStoredFilters(storeKey) {
        ...
        return this._userConfigRepository.search(criteria, Shopwell.Context.api).then((response) => {
            if (response.length) {
                this._filterEntity = response.first();
            } else {
                const currentUser = Shopwell.Store.get('session').currentUser;

                this._filterEntity = this._userConfigRepository.create(Shopwell.Context.api);
    ...
    saveFilters(storeKey, filters) {
        ...
        this._userConfigRepository.save(this._filterEntity, Shopwell.Context.api).then(() => {
    ...
    async _pushFiltersToUrl(replaceRoute = false) {
        ...
        const router = Shopwell.Application.view.router;
        ...
        if (!Shopwell.Utils.types.isEmpty(routeParams)) {
