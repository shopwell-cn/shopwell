...
export default function SearchPreferencesService({ userConfigRepository: _userConfigRepository }) {
    ...
    function getDefaultSearchPreferences() {
        ...
        Shopwell.Module.getModuleRegistry().forEach(({ manifest }) => {
            if (
                manifest.entity &&
                Shopwell.Service('acl').can(`${manifest.entity}.editor`) &&
    ...
    function processSearchPreferencesFields(tempSearchPreferencesFields) {
        ...
        tempSearchPreferencesFields.forEach((field) => {
            field.group.forEach((group) => {
                const searchPreferencesField = Shopwell.Utils.object.set({}, group.fieldName, {
                    _searchable: field._searchable,
                    _score: field._score,
                });
                searchPreferencesFields = Shopwell.Utils.object.deepMergeObject(
    ...
    function _getUserConfigCriteria() {
        const criteria = new Shopwell.Data.Criteria();

        criteria.addFilter(Shopwell.Data.Criteria.equals('key', KEY_USER_SEARCH_PREFERENCE));
        criteria.addFilter(Shopwell.Data.Criteria.equals('userId', _getCurrentUser()?.id));
