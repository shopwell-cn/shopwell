...
export default function initializeTabs(): void {
    Shopwell.ExtensionAPI.handle('uiTabsAddTabItem', async (componentConfig) => {
        Shopwell.Store.get('tabs').addTabItem(componentConfig);
    ...
    void Shopwell.Application.viewInitialized.then(() => {
        ...
        router.beforeResolve((to) => {
            ...
            // Get all tab routes
            const tabRoutes = Object.values(Shopwell.Store.get('tabs').tabItems).reduce<string[]>((acc, tabItems) => {
                acc = [
                    ...acc,
            ...
            const moduleInfo = Shopwell?.Application?.$container?.container?.init?.router?.getModuleInfo?.(parentRoute) as
                | {
            ...
            const newRoute: RouteRecordRaw = {
                path: newPath,
                component: Shopwell.Application.view?.getComponent('sw-extension-component-section'),
                children: [],
                name: newRouteName,
                meta: {
