...
import type { ApiContext } from '@shopwell-ag/meteor-admin-sdk/es/_internals/data/EntityCollection';
...
export default function initializeWorker() {
    ...
    const context = Shopwell.Context.app;
    const configService = Shopwell.Service('configService');

    if (!Shopwell.Feature.isActive('v6.8.0.0')) {
        const workerNotificationFactory = Shopwell.Application.getContainer('factory').workerNotification;
        ...
        return configService.getConfig().then((response) => {
            Object.entries(response as ContextAppConfig).forEach(
                ([
                    ...
                    Shopwell.Store.get('context').addAppConfigValue({
            ...
            if (!Shopwell.Feature.isActive('v6.8.0.0')) {
                enableWorkerNotificationListener(loginService, Shopwell.Context.api);
            }
            ...
            if (context.config.adminWorker?.enableAdminWorker && !enabled) {
                enableAdminWorker(loginService, Shopwell.Context.api, context.config.adminWorker);
            }
        });
    }
...
function enableAdminWorker(
    ...
    const importExportService = Shopwell.Service('importExport');
...
function registerThumbnailMiddleware(factory: typeof WorkerNotificationFactory) {
    ...
    factory.register('EsIndexingMessage', {
        name: 'Shopwell\\Elasticsearch\\Framework\\Indexing\\IndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
    ...
    factory.register('generateThumbnailsMessage', {
        name: 'Shopwell\\Core\\Content\\Media\\Message\\GenerateThumbnailsMessage',
        fn: function middleware(next, { entry, $root, notification }) {
    ...
    factory.register('PromotionIndexingMessage', {
        name: 'Shopwell\\Core\\Checkout\\Promotion\\DataAbstractionLayer\\PromotionIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
    ...
    factory.register('ProductStreamIndexingMessage', {
        name: 'Shopwell\\Core\\Content\\ProductStream\\DataAbstractionLayer\\ProductStreamIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
            messageQueueNotification(
                'productStream',
                ids,
    ...
    factory.register('CategoryIndexingMessage', {
        name: 'Shopwell\\Core\\Content\\Category\\DataAbstractionLayer\\CategoryIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
            messageQueueNotification(
                'category',
                ids,
    ...
    factory.register('MediaIndexingMessage', {
        name: 'Shopwell\\Core\\Content\\Media\\DataAbstractionLayer\\MediaIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
            messageQueueNotification(
                'media',
                ids,
    ...
    factory.register('SalesChannelIndexingMessage', {
        name: 'Shopwell\\Core\\System\\SalesChannel\\DataAbstractionLayer\\SalesChannelIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
            messageQueueNotification(
                'salesChannel',
                ids,
    ...
    factory.register('RuleIndexingMessage', {
        name: 'Shopwell\\Core\\Content\\Rule\\DataAbstractionLayer\\RuleIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
            messageQueueNotification(
                'rule',
                ids,
    ...
    factory.register('ProductIndexingMessage', {
        name: 'Shopwell\\Core\\Content\\Product\\DataAbstractionLayer\\ProductIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
            messageQueueNotification(
                'product',
                ids,
    ...
    factory.register('ElasticSearchIndexingMessage', {
        name: 'Shopwell\\Elasticsearch\\Framework\\Indexing\\ElasticsearchIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
            messageQueueNotification(
                'esIndexing',
                ids,
    ...
    factory.register('ImportExportMessage', {
        name: 'Shopwell\\Core\\Content\\ImportExport\\Message\\ImportExportMessage',
        fn: function middleware(next, { entry, $root, notification }) {
    ...
    factory.register('FlowIndexingMessage', {
        name: 'Shopwell\\Core\\Content\\Flow\\Indexing\\FlowIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
    ...
    factory.register('NewsletterRecipientIndexingMessage', {
        name: 'Shopwell\\Core\\Content\\Newsletter\\DataAbstractionLayer\\NewsletterRecipientIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
