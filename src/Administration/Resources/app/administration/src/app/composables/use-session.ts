...
async function setAdminLocale(locale: string): Promise<void> {
    ...
    const localeToLanguageService = Shopwell.Service('localeToLanguageService') as LocaleToLanguageService;
    return localeToLanguageService.localeToLanguage(locale).then((languageIdFromService) => {
        ...
        Shopwell.Application.getContainer('factory').locale.storeCurrentLocale(locale);
    });
}
...
function setAdminLocaleState({
    ...
    if (!locales.find((l) => l === locale)) {
        Shopwell.Utils.debug.warn('SessionStore', `Locale ${locale} not registered at store`);
        return;
    }
