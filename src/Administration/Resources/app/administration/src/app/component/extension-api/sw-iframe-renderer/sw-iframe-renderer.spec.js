...
describe('src/app/component/extension-api/sw-iframe-renderer', () => {
    beforeEach(async () => {
        ...
        Object.keys(Shopwell.Store.get('extensions').extensionsState).forEach((key) => {
            delete Shopwell.Store.get('extensions').extensionsState[key];
        });

        // Clear sdkLocation store
        Object.keys(Shopwell.Store.get('sdkLocation').locations).forEach((key) => {
            delete Shopwell.Store.get('sdkLocation').locations[key];
        });
