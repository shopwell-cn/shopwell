...
export default Shopwell.Component.wrapComponentConfig({
    ...
    created() {
        this.heightHandler = Shopwell.ExtensionAPI.handle('locationUpdateHeight', ({ height, locationId }) => {
            if (locationId === this.locationId) {
                this.locationHeight = Number(height) ?? null;
            }
        });

        this.urlHandler = Shopwell.ExtensionAPI.handle(
            'locationUpdateUrl',
    ...
    computed: {
        ...
        componentName(): string | undefined {
            return Shopwell.Store.get('sdkLocation').locations[this.locationId];
        },

        extension(): Extension | undefined {
            const extensions = Shopwell.Store.get('extensions').extensionsState;
