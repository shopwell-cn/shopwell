...
const { dom, types } = Shopwell.Utils;
...
export default {
    ...
    computed: {
        ...
        currentLocale() {
            return Shopwell.Store.get('session').currentLocale;
        },
...
The admin menu only supports up to three levels of nesting.`,
        ...
        mainMenuEntries() {
            const tree = new Shopwell.Helper.FlatTreeHelper((first, second) => first.position - second.position);
        ...
        extensionMenuItems() {
            return Shopwell.Store.get('menuItem').menuItems;
        },
        ...
        adminMenuStore() {
            return Shopwell.Store.get('adminMenu');
        },
    },
    ...
    methods: {
        createdComponent() {
            ...
            Shopwell.Utils.EventBus.on('sw-admin-menu/toggle-offcanvas', this.onToggleCanvas);

            this.initNavigation();
        },

        beforeUnmountedComponent() {
            Shopwell.Utils.EventBus.off('sw-admin-menu/toggle-offcanvas', this.onToggleCanvas);
        },
        ...
        getUser() {
            ...
            this.userService.getUser().then((response) => {
                ...
                Shopwell.Store.get('session').setCurrentUser(userData);
        ...
        onLogoutUser() {
            ...
            Shopwell.Store.get('notification').clearGrowlNotificationsForCurrentUser();
