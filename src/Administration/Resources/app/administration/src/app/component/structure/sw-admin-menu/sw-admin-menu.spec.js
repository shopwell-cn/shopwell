...
const menuService = createMenuService(Shopwell.Module);
Shopwell.Service().register('menuService', () => menuService);

async function createWrapper(options = {}) {
    const router = createRouter({
        routes: [
            ...Shopwell.Module.getModuleRoutes(),
            {
...
describe('src/app/component/structure/sw-admin-menu', () => {
    ...
    beforeAll(() => {
        ...
        Shopwell.Module.getModuleRegistry().clear();
        adminModules.forEach((adminModule) => {
            Shopwell.Module.register(adminModule.name, adminModule);
        });
    });
    ...
    it('should not render 4.level or higher menu item and throw error', async () => {
        ...
        expect(Shopwell.Utils.debug.error.mock.calls[0][0]).toBeInstanceOf(Error);
        expect(Shopwell.Utils.debug.error.mock.calls[0][0].toString()).toBe(
        ...
        expect(Shopwell.Utils.debug.error.mock.calls[1][0]).toBeInstanceOf(Error);
        expect(Shopwell.Utils.debug.error.mock.calls[1][0].toString()).toBe(
