...
describe('src/app/component/form/sw-datepicker', () => {
    ...
    beforeEach(async () => {
        Shopwell.Store.get('session').setCurrentUser({ timeZone: 'UTC' });
    });

    afterAll(() => {
        Shopwell.Store.get('session').setCurrentUser(currentUser);
    });
    ...
    it.each([
        ...
        async ({ dateType, timeZone, expectedTimeZone }) => {
            Shopwell.Store.get('session').setCurrentUser({ timeZone: timeZone });
    ...
    it('should not convert the date when a timezone is set and dateType is date', async () => {
        Shopwell.Store.get('session').setCurrentUser({ timeZone: 'Europe/Berlin' });
    ...
    it('should support other locales formats', async () => {
        Shopwell.Store.get('session').currentLocale = 'en-US';
        wrapper = await createWrapper({});
        ...
        Shopwell.Store.get('session').currentLocale = 'en-UK';
