 ...
 */
import type ShopwellError from 'src/core/data/ShopwellError';
 ...
 * */
function setApiErrorNested({ expression, error, store }: { expression: string; error: ShopwellError; store: ErrorStore }) {
...
function removeApiError(expression: string, store: ErrorStore) {
    ...
    if (Shopwell.Utils.types.isEmpty(store)) {
 ...
 */
const errorStore = Shopwell.Store.register({
    id: 'error',
    ...
    state: () =>
        ({
            ...
            system: Record<string, ShopwellError>;
            api: Record<string, ShopwellError>;
        },

    actions: {
        addApiError({ expression, error }: { expression: string; error: ShopwellError }) {
        ...
        addSystemError({ error, id = utils.createId() }: { error: ShopwellError; id?: string }) {
    ...
    getters: {
        getErrorsForEntity(state) {
            return (entityName: string, id: string) => {
                ...
                // @ts-expect-error
                return (entityStorage[id] as ShopwellError) ?? null;
            };
        },
