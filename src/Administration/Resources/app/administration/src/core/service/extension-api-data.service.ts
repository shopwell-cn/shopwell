...
handleGet((data, additionalOptions) => {
    ...
    if (registeredDataSet.deprecated) {
        ...
        if (process.env.NODE_ENV !== 'production') {
            Shopwell.Utils.debug.error(...debugArgs);
        } else {
            Shopwell.Utils.debug.warn(...debugArgs);
        }
    }
    ...
    updateSubscriber(id, (value) => {
        ...
        function setObject(transferObject: transferObject, prePath: string | null = null): void {
            ...
            Object.keys(transferObject).forEach((property) => {
                ...
                if (
                    // @ts-expect-error
                    Shopwell.Utils.hasOwnProperty(transferObject[property], 'getDraft', this) &&
                    // @ts-expect-error
                    typeof transferObject[property].getDraft === 'function'
                ) {
                    setObject(
                        {
                            [property]: Shopwell.Utils.object.cloneDeep(transferObject[property]),
                        },
                        realPath,
                    );
                ...
                Shopwell.Utils.object.get(scope, parsedPath.pathToLastSegment)[parsedPath.lastSegment] =
        ...
        if (path.includes('.')) {
            ...
            // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
            Shopwell.Utils.object.get(scope, newPath)[lastPath] = value.data;
