...
describe('src/core/service/usage-data-consent-listener.service.ts', () => {
    ...
    beforeEach(() => {
        ...
        loginService = createLoginService({}, Shopwell.Context.api);
        serviceContainer = {
            usageDataService: {
                getConsent: () => {
        ...
        Shopwell.Store.get('usageData').updateConsent({
    ...
    it('should update consent on login', async () => {
        ...
        expect(Shopwell.Store.get('usageData').isConsentGiven).toBe(true);
        expect(Shopwell.Store.get('usageData').isBannerHidden).toBe(false);
    });
    ...
    it('should reset the consent if the request fails', async () => {
        ...
        expect(Shopwell.Store.get('usageData').isConsentGiven).toBe(false);
        expect(Shopwell.Store.get('usageData').isBannerHidden).toBe(true);
    });
