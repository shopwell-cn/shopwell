...
export default function addPluginUpdatesListener(loginService: LoginService, serviceContainer: ServiceContainer): void {
    ...
    function createUpdatesAvailableNotification(): void {
        const root = Shopwell.Application.getApplicationRoot();
        ...
        void Shopwell.Store.get('notification').createNotification(notification);
    }
    ...
    Shopwell.Vue.watch(useSession().currentUser, (newValue) => {
        if (!newValue) {
            return;
        }
