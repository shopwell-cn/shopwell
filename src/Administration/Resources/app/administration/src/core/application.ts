...
class ApplicationBootstrapper {
     ...
     *
     * @example
     * Shopwell.Application.addFactory('module', (container) => {
     ...
     *
     * @example
     * Shopwell.Application.addFactoryMiddleware((container, next) => {
     *    // Do something with the container
     *    next();
     * });
     *
     * @example
     * Shopwell.Application.addFactoryMiddleware('module', (service, next) => {
     ...
     *
     * @example
     * Shopwell.Application.addFactoryDecorator((container, next) => {
     *    // Do something with the container
     *    next();
     * });
     *
     * @example
     * Shopwell.Application.addFactoryDecorator('module', (service, next) => {
     *    // Do something with the service
     *    next();
     * });
     */
     ...
     *
     * @example
     * Shopwell.Application.addInitializer('httpClient', (container) => {
     ...
     *
     * @example
     * Shopwell.Application.addServiceProvider('productService', (container) => {
    ...
    registerApiContext(context: ContextStore['api']): ApplicationBootstrapper {
        Shopwell.Context.api = Shopwell.Classes._private.ApiContextFactory(context);
    ...
    registerAppContext(context: ContextStore['app']): ApplicationBootstrapper {
        Shopwell.Context.app = Shopwell.Classes._private.AppContextFactory(context);
     ...
     *
     * @example
     * Shopwell.Application.addServiceProviderMiddleware((container, next) => {
     *    // Do something with the container
     *    next();
     * });
     *
     * @example
     * Shopwell.Application.addServiceProviderMiddleware('productService', (service, next) => {
     *    // Do something with the service
     *    next();
     * });
     */
    ...
    initializeFeatureFlags(): ApplicationBootstrapper {
        const features = Shopwell.Context.app.features;
        ...
        Shopwell.Feature.init(features);
     ...
     *
     * @example
     * Shopwell.Application.addServiceProviderDecorator((container, next) => {
     *    // Do something with the container
     *    next();
     * });
     *
     * @example
     * Shopwell.Application.addServiceProviderDecorator('productService', (service, next) => {
     *    // Do something with the service
     *    next();
     * });
     */
    ...
    createApplicationRoot(): Promise<ApplicationBootstrapper> {
        ...
        if (Shopwell.Context.app.environment === 'testing') {
    ...
    private async initializeLoginInitializer(): Promise<unknown[]> {
        ...
        await Shopwell.Application.getContainer('init-pre').apiServices;
    ...
    private async loadPlugins(): Promise<(unknown[] | null)[]> {
        ...
        if (isDevelopmentMode) {
            ...
            if (Shopwell.Utils.object.hasOwnProperty(plugins, 'metadata')) {
                delete plugins.metadata;
            }
        } else {
            // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
            plugins = Shopwell.Context.app.config.bundles as bundlesPluginResponse;
        }
        ...
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
        const bundles = Shopwell.Context.app.config.bundles as bundlesPluginResponse;
                ...
                if (isDevelopmentMode) {
                    ...
                    Object.entries(plugins).forEach(
                        ([
                            ...
                            const stringUtils = Shopwell.Utils.string;
    ...
    private injectIframe({
        ...
        const bundles = Shopwell.Context.app.config.bundles;
        ...
        Shopwell.Store.get('extensions').addExtension(extension);
    }
}
