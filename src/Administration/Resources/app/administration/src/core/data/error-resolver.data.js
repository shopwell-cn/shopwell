...
export default class ErrorResolver {
    ...
    resetApiErrors() {
        return Shopwell.Store.get('error').resetApiErrors();
    }
    ...
    reduceErrorsByWriteIndex(errors) {
        ...
        errors.forEach((current) => {
            ...
            segments.reduce((pointer, segment, index) => {
                ...
                if (index === lastIndex) {
                    pointer[segment] = new this.ShopwellError(current);
                } else {
                    pointer[segment] = {};
                }
    ...
    addSystemErrors(systemErrors) {
        systemErrors.forEach((error) => {
            Shopwell.Store.get('error').addSystemError(error);
        });
    }
    ...
    resolveError(fieldName, error, definition, entity, changeset) {
        ...
        if (!(error instanceof this.ShopwellError)) {
            error = new this.ShopwellError(error);
        }
    ...
    buildAssociationChangeset(entity, changeset, error, associationName) {
        if (!changeset || !Shopwell.Utils.object.hasOwnProperty(changeset, associationName)) {
            Shopwell.Store.get('error').addApiError({
    ...
    resolveJsonFieldError(basePath, error) {
        Object.keys(error).forEach((fieldName) => {
            ...
            if (error[fieldName] instanceof this.ShopwellError) {
                Shopwell.Store.get('error').addApiError({
                    expression: path,
