...
describe('src/core/data/error-resolver.data', () => {
    ...
    beforeEach(() => {
        ...
        jest.spyOn(Shopwell.Store.get('error'), 'resetApiErrors');
        jest.spyOn(Shopwell.Store.get('error'), 'addApiError');
        jest.spyOn(Shopwell.Store.get('error'), 'addSystemError');
    ...
    describe('resetApiErrors', () => {
        it('should dispatches "error/resetApiErrors" action', () => {
            ...
            expect(Shopwell.Store.get('error').resetApiErrors).toHaveBeenCalledTimes(1);
        });
    });
    ...
    describe('handleWriteErrors', () => {
        ...
        it('should handles write errors and adds system errors', () => {
            ...
            expect(Shopwell.Store.get('error').addApiError).toHaveBeenCalledTimes(2);
            expect(Shopwell.Store.get('error').addSystemError).toHaveBeenCalledTimes(1);
            expect(Shopwell.Store.get('error').addApiError).toHaveBeenNthCalledWith(1, {
                expression: expect.anything(),
                error: expect.any(Shopwell.Classes.ShopwellError),
            });
            expect(Shopwell.Store.get('error').addApiError).toHaveBeenNthCalledWith(2, {
                expression: expect.anything(),
                error: expect.any(Shopwell.Classes.ShopwellError),
            });
            expect(Shopwell.Store.get('error').addSystemError).toHaveBeenCalledWith(
                expect.any(Shopwell.Classes.ShopwellError),
            );
        });
        ...
        it('should convert to ShopwellError', () => {
            ...
            expect(Shopwell.Store.get('error').addApiError).toHaveBeenCalledWith({
                expression: expect.anything(),
                error: expect.any(Shopwell.Classes.ShopwellError),
            });
        });
    ...
    describe('handleDeleteError', () => {
        it('should handle delete errors and add system errors and api errors', () => {
            ...
            expect(Shopwell.Store.get('error').addSystemError).toHaveBeenCalledTimes(2);
            expect(Shopwell.Store.get('error').addApiError).toHaveBeenCalledTimes(2);
            expect(Shopwell.Store.get('error').addSystemError).toHaveBeenCalledWith({
                error: expect.any(Shopwell.Classes.ShopwellError),
            });
