...
function getModuleRegistry(): Map<string, ModuleDefinition> {
    modules.forEach((value, key) => {
        if (hasOwnProperty(value.manifest, 'flag') && !Shopwell.Feature.isActive(value?.manifest?.flag ?? '')) {
            modules.delete(key);
        }
    });
...
function getModuleRoutes(): SwRouteConfig[] {
    ...
    modules.forEach((module) => {
        module.routes.forEach((route) => {
            if (hasOwnProperty(route, 'flag') && !Shopwell.Feature.isActive(route.flag ?? '')) {
                return;
            }
...
function addSettingsItemsToStore(moduleId: string, module: ModuleManifest): void {
    if (hasOwnProperty(module, 'flag') && !Shopwell.Feature.isActive(module.flag ?? '')) {
        return;
    }
...
function addEntryRouteToExtensionRouteStore(config: { extensionName: string; route: string }): void {
    ...
    Shopwell.Store.get('extensionEntryRoutes').addItem(config);
}
