...
export default {
    ...
    handle: <MESSAGE_TYPE extends keyof ShopwellMessageTypes>(
        type: MESSAGE_TYPE,
        ...
        const aclHook = (
            ...
            return new Promise((resolve, reject) => {
                ...
                const missingPrivileges = data.privileges!.filter((p) => !Shopwell.Service('acl').can(p));
                if (missingPrivileges.length > 0) {
                    reject(new MissingPrivilegesError(type, missingPrivileges));
                } else {
                    const result = method(data, additionalInformation);

                    if (isPromise<ShopwellMessageTypes[MESSAGE_TYPE]['responseType']>(result)) {
