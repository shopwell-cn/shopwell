...
class Migration1773153289PaymentSystemGatewayConfig extends MigrationStep
{
    ...
    {
        $sql = <<<'SQL'
        CREATE TABLE IF NOT EXISTS `payment_system_gateway_config` (
              ...
              PRIMARY KEY (`id`),
              UNIQUE INDEX `uniq.payment_system_gateway_config.name` (`name`),
              CONSTRAINT `json.payment_system_gateway_config.config` CHECK (JSON_VALID(`config`))
            ) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
SQL;
        $connection->executeStatement($sql);

        $hasData = $connection->executeQuery('SELECT 1 FROM `payment_system_gateway_config` LIMIT 1')->fetchAssociative();
        if ($hasData) {
            return;
        }
