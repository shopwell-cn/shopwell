...
namespace Shopwell\Core\Maintenance;

use Shopwell\Core\Framework\HttpException;
use Shopwell\Core\Framework\Log\Package;
use Shopwell\Core\Framework\Migration\MigrationCollectionLoader;
use Shopwell\Core\Maintenance\System\Exception\DatabaseSetupException;
...
class MaintenanceException extends HttpException
{
    ...
    public static function dbVersionMismatch(
        ...
        return new DatabaseSetupException(
            Response::HTTP_INTERNAL_SERVER_ERROR,
            self::MAINTENANCE_DB_VERSION_SELECT_FAILED,
            'Your database server is running {{ dbKind }} {{ actualVersion }}, but Shopwell 6 requires at least MySQL {{ mysqlRequiredVersion }} OR MariaDB {{ mariaDBRequiredVersion }}',
            [
                'dbKind' => $dbKind,
