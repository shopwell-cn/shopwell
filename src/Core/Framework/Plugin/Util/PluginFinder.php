...
use Shopwell\Core\Framework\Plugin\Exception\ExceptionCollection;
use Shopwell\Core\Framework\Plugin\Exception\PluginComposerJsonInvalidException;
use Shopwell\Core\Framework\Plugin\Struct\PluginFromFileSystemStruct;
...
class PluginFinder
{
    ...
    private function loadLocalPlugins(string $pluginDir, IOInterface $composerIO, ExceptionCollection $errors): array
    {
        ...
        try {
            ...
            foreach ($filesystemPlugins as $filesystemPlugin) {
                ...
                if (!$this->isShopwellPluginType($package) || !$this->isPluginComposerValid($package)) {
    ...
    private function isShopwellPluginType(CompletePackageInterface $package): bool
    {
    ...
    private function enrichWithVendorPlugins(
        ...
        foreach ($composerPackages as $composerPackage) {
            if (!$this->isShopwellPluginType($composerPackage)) {
                continue;
            }
