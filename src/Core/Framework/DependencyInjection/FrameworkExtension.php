...
class FrameworkExtension extends Extension
{
    ...
    public function load(array $configs, ContainerBuilder $container): void
    {
        ...
        $this->addShopwellConfig($container, $this->getAlias(), $config);
    }

    private function addShopwellConfig(ContainerBuilder $container, string $alias, array $options): void
    {
        foreach ($options as $key => $option) {
            ...
            if (\is_array($option)) {
                $this->addShopwellConfig($container, $key, $option);
            }
        }
    }
}
