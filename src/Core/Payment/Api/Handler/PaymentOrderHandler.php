...
use Shopwell\Core\Payment\Api\Extension\PaymentPlaceOrderExtension;
use Shopwell\Core\Payment\Api\PaymentApiException;
use Shopwell\Core\Payment\Api\Response\PaymentOrderResponse;
use Shopwell\Core\Payment\Api\Struct\PaymentOrderPlaceResult;
use Shopwell\Core\Payment\Order\PaymentOrderCollection;
...
readonly class PaymentOrderHandler
{
    ...
    public function createOrder(DataBag $request, Context $context): PaymentOrderResponse
    {
        $placed = $this->extensions->publish(
            name: PaymentPlaceOrderExtension::NAME,
            extension: new PaymentPlaceOrderExtension($request, $context),
            function: $this->place(...)
        );
        ...
        if (!$orderEntity) {
            throw PaymentApiException::invalidPaymentOrderNotStored($orderId);
        }
