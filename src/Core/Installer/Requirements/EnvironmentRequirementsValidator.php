...
class EnvironmentRequirementsValidator implements RequirementsValidatorInterface
{
    ...
    {
        $platform = $this->composer->getRepositoryManager()->getLocalRepository()->findPackage('shopwell/platform', '*');
        if (!$platform) {
            $platform = $this->composer->getRepositoryManager()->getLocalRepository()->findPackage('shopwell/core', '*');
        }
        if (!$platform) {
